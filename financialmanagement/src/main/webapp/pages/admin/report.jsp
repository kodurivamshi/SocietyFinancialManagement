...
<%@page import="com.app.models.ExpenditureModel"%>
...
ArrayList<ExpenditureModel>em=new ArrayList<ExpenditureModel>();
...
if(session.getAttribute("admin")==null && session.getAttribute("uname")==null)
	...
	total=rs.getMonthlyExpenditure();
	em=rs.getDetailedMonthlyExpenditure();
}
%>
<%
...
<div class="accordion" id="accordionExample">
  ...
  <tbody>
	 ...
	 <div>Worker's fee includes the <i><b>security  salary</b></i> <%=m.getSecurityFee() %> and  <i><b>Cleaner salary </b> </i> <%=m.getCleanerFee() %> . </div>
	&nbsp<div>Here Good's fee includes all the expenditure on cleaning items.</div>
	</div>
	<div>Below table shows the <b>OutWard Payments</b> done by society as a part of savings fee. </div>
	   <div class="row" style="width:auto;padding:10px;margin-left:10px;">
		<span class="badge bg-danger" style="color:white">OutWard Payments by Society</span><br><br>
		<table id="rents" class="table table-hover table-primary">
  <thead>
    <tr class="table-secondary">
      <th>S.NO</th>
  ...
  <tbody>
  		...
  		<tr><td><b><%=i+1%></b></td><td><%=em.get(i).getHeading()%></td><td><%=em.get(i).getAmount()%></td></tr>
  		<%} %>
    </tbody>
</table>
	<div>Therefore the total expenditure spent on OutWard payments is <%=total.get(1) %> &nbsp & on clearing raised Clompaints is <%=total.get(0) %></div>
	</div>
